...
export async function POST(request: Request) {
  try {
    ...
    const agentPrompt = await db.agentPrompts.create({
      data: {
        name,
        content,
      },
...
export async function PUT(request: Request) {
  try {
    ...
    const existingPrompt = await db.agentPrompts.findUnique({
      where: { id: Number(id) },
    });
    ...
    const updatedPrompt = await db.agentPrompts.update({
      where: { id: Number(id) },
      data: {
        ...(name && { name }),
        ...(content && { content }),
...
export async function GET(request: Request) {
  try {
    ...
    const prompt = await db.agentPrompts.findFirst({
      where: { name },
      select: {
        content: true,
      },
